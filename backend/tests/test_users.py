...
def test_create_user(client):
    res = client.post('/api/v1/users/', json={"email": "test_create@gmail.com", "password": "password123"})
...
def test_login(client, test_user):
    res = client.post('/api/v1/login', {'username': test_user['email'], 'password': test_user['password']})
...
def test_incorrect_login(client, test_user, email, password, status_code):
    res = client.post('/api/v1/login', {'username': email, 'password': password})
    assert res.status_code == status_code


def test_get_new_access_token_from_refresh(authorised_client, refresh_token, test_user):
    res = authorised_client.post('/api/v1/refresh', json={'refresh_token': refresh_token})
